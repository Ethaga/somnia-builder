...
export default function ERC20Tools() {
  ...
  const { data: symbol } = useReadContract({
    abi: MIN_ABI,
    address: token && /^0x[a-fA-F0-9]{40}$/.test(token) ? (token as Address) : undefined,
    functionName: "symbol",
  ...
  const { data: balance } = useReadContract({
    ...
    query: { enabled: !!token && /^0x[a-fA-F0-9]{40}$/.test(token) && !!wallet },
  });
      ...
      <CardContent>
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <Input id="token" placeholder="0x..." value={token} onChange={(e) => setToken(e.target.value)} />
          </div>
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div>
              ...
              <div className="font-medium">{decimals !== undefined ? String(decimals) : "—"}</div>
            </div>
            <div className="col-span-2">
              <div className="text-muted-foreground">Your Balance</div>
              <div className="font-medium">{formattedBalance ? formattedBalance : "—"}</div>
            </div>
          </div>
          <div className="pt-2">
